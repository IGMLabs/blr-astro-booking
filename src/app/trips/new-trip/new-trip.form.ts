...
import { FormBuilder, FormControl, Validators, AbstractControl, FormGroup, ValidationErrors } from '@angular/forms';
...
export class NewTripForm implements OnInit {
  ...
  constructor(formBuilder: FormBuilder) {
    this.form = formBuilder.group({
      ...
      plazas: new FormControl('', [Validators.required, Validators.min(1), Validators.max(10)]),
      precio: new FormControl('', [Validators.required, Validators.min(1000000), Validators.max(10000000)]),
  ...
  public getErrorMessage(controlName: string): string {
    ...
    errorMessage += errors['minlength']
      ...
      : ' ';
      errorMessage += errors['min'] ? `🔥 Field needs more ${errors['min'].min} ` : ' ';
      errorMessage += errors['max'] ? `🔥 Field needs less ${errors['max'].max}` : ' ';
    return errorMessage;
  }
  onSave() {
    const { destination, agencyId, plazas, precio, dateFrom, dateTo } = this.form.value;
    const id = this.getDashId(destination, agencyId );
    const newTripData = { id,destination, agencyId, plazas, precio, dateFrom, dateTo};
  ...
  private getDashId(destino: string , id:string): string {
    ...
    getDateMessage(){
      ...
      if(!errors){return ''};
      if(errors['dateControl']){return errors ['dateControl']};
      return '';
    }
